...
do
        ...
        grep -rn object $i/ls0.dx > $i/testfile1.XXXX
        ...
        # 5. 选择水平集函数 分割前两部分 获得 node.txt 和 mesh.txt 文件；
        # 这里空格必不可少 expr后面的空格
        sed -n "`expr ${raw0} + 1`,`expr ${raw1} - 1`p" $i/ls0.dx >$i/node.txt
        sed -n "`expr ${raw1} + 1`,`expr ${raw2} - 4`p" $i/ls0.dx >$i/mesh.txt
        ...
        # 水平集函数需要单独处理
        wc -l $i/ls0.dx > $i/tempfile
        lastlineNum=`cut -d" " -f1 $i/tempfile`

        sed -n "`expr ${raw2} + 1`,`expr ${lastlineNum} - 7`p" $i/ls0.dx >$i/ls0.txt
        sed -n "`expr ${raw2} + 1`,`expr ${lastlineNum} - 7`p" $i/ls1.dx >$i/ls1.txt
        # 用完删除
        rm -f $i/tempfile
done
