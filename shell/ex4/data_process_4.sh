...
# 并行程序
# 输入参数1 2 3 4 5 分别是起始 终止  步长 相的数量 和 进程数量 
#输入程序：
#1. 水平集函数
#	lsX_Y_PZ.dx  X是0或1；表示当前步和上一步的水平集函数 Y是进程号 Z是相的编号
#2. 各相守恒量 守恒量分别是 E rho rho_u rho_v 能量 密度 和动量
#	守恒量_Z_Y.dx Z是0到相-1；表示第Z相 Y是进程号
#
#输出在各步目录下 包括行号 和 节点 网格 和 函数值 数据，后者后缀是txt
#输出程序：
#	1. RawNumber_Y 行号 Y是进程号
#	2. 守恒量_Z_Y.txt Z是0到相-1；表示第Z相 Y是进程号
#	   水平集函数 lsX_Y_PZ.dx  X是0或1；表示当前步和上一步的水平集函数 Y是进程号 Z是相的编号
#   3. node_Y.txt mesh_Y.txt Y是进程号
...
do
    ...
    do
        ...
        grep -rn object $i/ls0_${j}_P0.dx > $i/testfile1.XXXX
        ...
        # 5. 选择水平集函数 分割前两部分 获得 node.txt 和 mesh.txt 文件；
        # 这里空格必不可少 expr后面的空格
        sed -n "`expr ${raw0} + 1`,`expr ${raw1} - 1`p" $i/ls0_${j}_P0.dx >$i/node_${j}.txt
        sed -n "`expr ${raw1} + 1`,`expr ${raw2} - 4`p" $i/ls0_${j}_P0.dx >$i/mesh_${j}.txt
        ...
        # 水平集函数需要单独处理
        wc -l $i/ls0_${j}_P0.dx > $i/tempfile
        lastlineNum=`cut -d" " -f1 $i/tempfile`
        for((k=0;k<${phase};k=k+1))
        do
            sed -n "`expr ${raw2} + 1`,`expr ${lastlineNum} - 7`p" $i/ls0_${j}_P${k}.dx >$i/ls0_${j}_P${k}.txt
            sed -n "`expr ${raw2} + 1`,`expr ${lastlineNum} - 7`p" $i/ls1_${j}_P${k}.dx >$i/ls1_${j}_P${k}.txt
        done
        # 用完删除
    rm -f $i/tempfile
    done
